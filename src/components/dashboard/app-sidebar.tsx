...
export function AppSidebar() {
  ...
  const filteredItems = navItems.filter(item => 
    !item.roles || (role && item.roles.includes(role)) || !role // Show all if no role (development fallback)
  )
      ...
      <SidebarContent className="px-3 pt-4">
        <SidebarGroup>
          <SidebarGroupLabel className="px-3 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/50 mb-2">
            {role === 'customer' ? 'Guest Lounge' : role === 'admin' || role === 'staff' ? 'Back of House' : 'Navigation'}
          </SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {filteredItems.map((item) => (
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <Link href={item.url} className="flex items-center w-full no-underline text-inherit hover:text-inherit">
        ...
        <SidebarMenu>
          {(!role || role === 'admin') && (
            <SidebarMenuItem>
              ...
              >
                <Link href="/dashboard/settings" className="no-underline text-inherit hover:text-inherit">
                  <Settings />
